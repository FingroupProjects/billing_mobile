...
class _OrganizationDetailsScreenState extends State<OrganizationDetailsScreen> {
  ...
  void initState() {
    super.initState();
    context.read<OrganizationByIdBloc>().add(FetchOrganizationByIdEvent(widget.clientId.toString()));
  }
  ...
  void _updateDetails(List<Organization> organizations) {
    if (organizations.isNotEmpty) {
      ...
      setState(() {
        details = [
          {'label': 'ID:', 'value': org.id.toString()},
          {'label': 'Название:', 'value': org.name},
          {'label': 'ИНН:', 'value': org.INN.toString()},
          {'label': 'Телефон:', 'value': org.phone},
          {'label': 'Адрес:', 'value': org.address},
          {'label': 'ID клиента:', 'value': org.clientId.toString()},
          {'label': 'Доступ:', 'value': org.hasAccess == 1 ? 'Есть' : 'Нет'},
          {
            'label': 'Создано:',
            'value': DateFormat('dd.MM.yyyy HH:mm').format(org.createdAt)
          },
          {
            'label': 'Обновлено:',
            'value': DateFormat('dd.MM.yyyy HH:mm').format(org.updatedAt)
          },
          {'label': 'Тип бизнеса:', 'value': org.businessTypeId.toString()},
          {
            'label': 'Причина отказа:',
            'value': org.rejectCause ?? 'Отсутствует'
          },
        ];
      });
    }
  ...
  AppBar _buildAppBar(BuildContext context, String title) {
    return AppBar(
      ...
      actions: [
        BlocBuilder<OrganizationByIdBloc, OrganizationByIdState>(
          builder: (context, state) {
            if (state is OrganizationByIdLoaded && state.organizations.isNotEmpty) {
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      showDialog(
                        context: context,
                        builder: (context) => DeleteOrganizationDialog(organizationId: organization.id),
                      ).then((deleted) {
                        if (deleted == true) {
