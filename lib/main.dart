...
void main() async {
  ...
  if (token != null) {
    try {
      user = await apiService.getUser(); // Предполагаемый метод для получения User
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: MaterialApp(
        ...
        home: token == null || user == null
            ? const LoginScreen()
            : HomeScreen(user: user!), // Передаём user
        routes: {
          '/login': (context) => const LoginScreen(),
          '/home': (context) => HomeScreen(user: user!), // Передаём user
        },
      ),
    );
  }
